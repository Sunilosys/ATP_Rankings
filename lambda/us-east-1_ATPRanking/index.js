...
var alexaSDK;
var serviceError = 'Sorry, We are not able to get the current ATP Rankings';
...
var cardTitle = "Men's Tennis ATP Rankings";
var goodByeMsg = 'Goodbye, Thank you for using the ATP Rankings skill';
var welcomeMsg = "Welcome to a. t. p. Rankings.\n";
var welcomeMsgCard = "Welcome to ATP Rankings.\n";
var helpMsg = "You can say any of the following.\n what is the current ranking of a particular player? For Example What is the current ranking of Roger Federer? \n" +
    "who is the current number one player? \n" +
    "who are the current top 10 players?";
var atpRankingEndPoint = "https://s3.amazonaws.com/atp-singles-ranking/ATP_Singles_Ranking.json";
var defaultTop = 10;
var maxTop = 25;
...
function handlePlayerRankingRequest(playerName) {
    makeHttpRequest(atpRankingEndPoint, function ResponseCallback(err, data) {
        var speechOutput = '';
        if (err) {
...
function handleTopRankingRequest(top) {
    makeHttpRequest(atpRankingEndPoint, function ResponseCallback(err, data) {
        var speechOutput = "Men's Tennis ATP Rankings Top " + top + ".\n ";
        if (err) {
            alexaSDK.emit(':tellWithCard', serviceError, cardTitle, serviceError, null);
        } else {
            var dataObj = JSON.parse(data);
